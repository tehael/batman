...
(function() {
  ...
  Batman.DOM.ViewArgumentBinding = (function(_super) {
    ...
    ViewArgumentBinding.prototype.die = function() {
      var arg, args, existingHandlers, handler, k, key, keypath, lookup, omg, val, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2;
      args = this.view.get('argumentBindings')._batman.properties.toArray();
      for (_i = 0, _len = args.length; _i < _len; _i++) {
        arg = args[_i];
        lookup = {};
        _ref = this.view._batman.properties.keys();
        ...
        for (key in lookup) {
          val = lookup[key];
          keypath = this.view.get("argumentBindings." + arg).keyPath;
          for (_k = 0, _len2 = val.length; _k < _len2; _k++) {
            omg = val[_k];
            existingHandlers = (_ref1 = this.view.get(arg).property(key).event('change', false)) != null ? _ref1.handlers : void 0;
            if (existingHandlers) {
              for (_l = 0, _len3 = existingHandlers.length; _l < _len3; _l++) {
                handler = existingHandlers[_l];
                if ((_ref2 = this.view.context.get(keypath).property(key).event('change', false)) != null) {
                  _ref2.removeHandler(handler);
                }
              }
            }
          }
