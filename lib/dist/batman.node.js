...
(function() {
  ...
  Batman.RailsStorage = (function(_super) {
    ...
    RailsStorage.prototype._addErrorsToRecord = function(givenErrors, record) {
      var association, errors, i, key, proxy, relatedErrors, relatedRecord, relatedRecords, validationError, _i, _j, _len, _len1, _ref;
      Batman.initializeObject(record.constructor);
      for (key in givenErrors) {
        errors = givenErrors[key];
        if (association = (_ref = record.constructor._batman.get('associations')) != null ? _ref.getByLabel(key) : void 0) {
          if (association.isSingular) {
            if (relatedRecord = association.getFromAttributes(record)) {
              this._addErrorsToRecord(errors, relatedRecord);
            } else if (proxy = record.associationProxy(key) && proxy.get('loaded')) {
              relatedRecord = proxy.get('target');
              this._addErrorsToRecord(proxy.get('target'), errors);
            }
          } else {
            relatedRecords = association.getFromAttributes(record) || association.setForRecord(record);
            if (relatedRecords.toArray != null) {
              relatedRecords = relatedRecords.toArray();
            }
            for (i = _i = 0, _len = errors.length; _i < _len; i = ++_i) {
              relatedErrors = errors[i];
              relatedRecord = relatedRecords[i];
              if (!relatedRecord) {
                continue;
              }
              this._addErrorsToRecord(relatedErrors, relatedRecord);
            }
          }
        } else {
          for (_j = 0, _len1 = errors.length; _j < _len1; _j++) {
            validationError = errors[_j];
    ...
    RailsStorage.prototype.after('update', 'create', function(env, next) {
      var error, record, response, validationErrors, _ref;
      record = env.subject;
      ...
      if (error) {
        if (((_ref = error.request) != null ? _ref.get('status') : void 0) === 422) {
          ...
          this._addErrorsToRecord(validationErrors, record);
          env.result = record;
